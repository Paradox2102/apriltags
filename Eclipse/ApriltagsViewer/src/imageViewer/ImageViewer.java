...
public class ImageViewer 
{
	...
	private JSpinner	m_whiteTarget		= null;
	private JSpinner	m_gain		 		= null;
	...
	private class CustomPanel extends JPanel
	{
	        ...
	        if (m_image != null)
	        {
	        	...
	        	if (m_regions != null)
	        	{
	        		...
	        		for (int i = 0 ; i < m_regions.m_nRegions ; i++)
	        		{
						...
						g2.drawString(String.format("%.1f",  region.m_distance), s(region.m_corners[3][0]), s(region.m_corners[3][1]+30));
		...
		Runnable doSetCameraData = new Runnable() 
		{
			public void run() 
			{
				...
				m_whiteTarget.setValue(m_data.m_whiteTarget);
				m_gain.setValue(m_data.m_gain);
				...
				if (m_autoExposure != null)
				{
					m_autoExposure.setSelected(m_data.m_autoExp);
				}
				...
				m_fps.setText(String.format("FPS:%.1f (%d,%d) B:%d C:%d V:%d AW:%d dt:%d mc:%d mdt:%d", m_data.m_fps, m_data.m_width, m_data.m_height,
	    ...
	    private void SetCameraData(Network.CameraData data)
	    {
	    	...
	    	{
	    		m_data	= data;
	    		m_panel.SetRegions(data.m_regions, 0, 0);
	    	}
	...
	public void DisplayImage(byte[] image, Network.CameraData cameraData)
	{
        ...
        try 
        {
            ...
            m_panel.SetCameraData(cameraData);
        } 
        catch (IOException e) 
        {
...
//	 PiCamera m_camera = new PiCamera();
	 ...
	 private void initPiCameraControls()
	 {
		if (!m_controlsInit)
		{
			...
			m_captureRate = new JSpinner();
			m_captureRate.setModel(new SpinnerNumberModel(Integer.valueOf(60), Integer.valueOf(1), Integer.valueOf(100), Integer.valueOf(10)));
			m_captureRate.addChangeListener(new ChangeListener() 
			{
				public void stateChanged(ChangeEvent arg0) 
				{
			...
			JLabel lblWhiteTarget = new JLabel("Brightness");
			lblWhiteTarget.setBounds(775, 66+dy, 100, 14);
			frame.getContentPane().add(lblWhiteTarget);
			...
			lblContrast.setBounds(665, 100+dy, 100, 14);
			frame.getContentPane().add(lblContrast);
			
			m_contrast = new JSpinner();
			m_contrast.setModel(new SpinnerNumberModel(Integer.valueOf(0), Integer.valueOf(-100), Integer.valueOf(100), Integer.valueOf(10)));
			...
			m_contrast.setBounds(665, 114+dy, 90, 20);
			...
			m_autoExposure.setBounds(773, 114+dy, 125, 23);
...
//		m_camera.connect("192.168.1.50", 5800);		// Testing
		...
		m_panel.addMouseListener(new MouseAdapter() 
		{
			...
			public void mouseReleased(MouseEvent event) 
			{
				...
				int displayWidth = m_panel.getWidth();
				int displayHeight = m_panel.getHeight();
...
//		initBWCameraControls();
		...
		lblFastTags.setBounds(655, dy+90, 104, 20);
		...
		lblFastTagsbox.setBounds(648, dy+100, 240, 125);
		...
		lblBlackColor.setBounds(665, dy+112, 100, 14);
		...
		m_whiteDrop.setBounds(665, dy+126, 90, 20);
		frame.getContentPane().add(m_whiteDrop);
		
		JLabel lblMinSize = new JLabel("Min Size");
		lblMinSize.setBounds(775, dy+112, 100, 14);
		...
		m_minSize.setBounds(775, dy+126, 90, 20);
		frame.getContentPane().add(m_minSize);

		JLabel lblMinBlack = new JLabel("Min Black");
		lblMinBlack.setBounds(665, dy+146, 100, 14);
		...
		m_minBlack.setBounds(665, dy+160, 90, 20);
		frame.getContentPane().add(m_minBlack);

		JLabel lblmaxSlope = new JLabel("Max Slope");
		lblmaxSlope.setBounds(665, dy+180, 100, 14);
		...
		m_maxParallel.setBounds(775, dy+194, 90, 20);
		frame.getContentPane().add(m_maxParallel);
		
		JLabel lblmaxAspect = new JLabel("Max Aspect");
		lblmaxAspect.setBounds(775, dy+146, 100, 14);
		...
		lblAprilTagsBox.setBounds(648, dy, 240, 90);
		...
		lblDecimate.setBounds(665, dy+10, 100, 14);
		...
		m_decimate.setBounds(665, dy+25, 90, 20);
		frame.getContentPane().add(m_decimate);

		JLabel lblThreads = new JLabel("# Threads");
		lblThreads.setBounds(775, dy+10, 100, 14);
		...
		m_threads.setBounds(775, dy+25, 90, 20);
		frame.getContentPane().add(m_threads);
		
		JLabel lblBlur = new JLabel("Blur");
		lblBlur.setBounds(665, dy+45, 100, 14);
		...
		m_blur.setBounds(665, dy+60, 90, 20);
		...
		m_refine.setBounds(775, dy+60, 133, 23);
		frame.getContentPane().add(m_refine);
		m_refine.setSelected(true);
		
		m_fps = new JLabel("FPS:");
		m_fps.setBounds(10, 492, 400, 14);
		frame.getContentPane().add(m_fps);
		...
		m_fast.setBounds(773, 420, 133, 23);
