...
public:
	...
	ImageRegion(int tag, double corners[4][2])
	{
		...
		solvePnP(m_corners, m_rvec, m_tvec);
	}
	...
	ImageRegion(ImageRegion * pSrc)
	{
		...
		memmove(m_rvec, pSrc->m_rvec, sizeof(m_rvec));
		memmove(m_tvec, pSrc->m_tvec, sizeof(m_tvec));
	}
};
...
extern void SaveImage(const char * prefix, unsigned char * pData);

extern float g_decimate;
extern float g_sigma;
extern bool g_refineEdges;
extern int g_nThreads;

extern void setDecimate(float decimate);
extern void setNThreads(int nThreads);
extern void setRefine(bool refine);
extern void setSigma(float sigma);
